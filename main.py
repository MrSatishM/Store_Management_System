...
class Application:
    def __init__(self,master, *args, **kwargs):
        ...
        #components
        self.heading =Label(self.left, text="Satish Retail Store", font=('arial 40 bold'))
        ...
        #fill it later by me by the function ajax
        self.productname = Label(self.left, text="", font=('arial 27 bold'), bg='white' , fg='lightblue')
        self.productname.place(x=0, y=250)

        self.pprice = Label(self.left, text="", font=('arial 27 bold'), bg='white' , fg='lightblue')
    ...
    def ajax(self, *args, **kwargs):
        ...
        #create the quantity & discount label
    ...
    def change_func(self, *args, **kwargs):
        #get the amount given by the customer and the amount generate by the computer
        self.amount_given = float(self.change_e.get())
        self.our_total  =float(sum(product_price))

        self.to_give = self.amount_give = self.amount_given - self.our_total
    ...
    def generate_bill(self, *args, **kwargs):
        ...
        directory ="C:/Users/satish maaurya/Desktop/Store Managemanet System/invoice/" + str(date) + "/"
        if not os.path.exists(directory):
            os.makedirs(directory)

        #templates for the bill
        company = "\t\t\t\t     SatishMaurya PvtLtd\n"
        address = "\t\t\t\t   Mumbai-78, Bhandup-(W).\n"
        phone = "\t\t\t\t         9892859841\n"
        sample ="\t\t\t\t          Invoice\n"
        dt = "\t\t\t\t\t   " + str(date)

        table_header = "\n\n\t\t\t\t----------------------------------\n\t\t\t\tSr.\tProducts\t    Qty\tAmount\t\t\t\t\t\t----------------------------------"
        ...
        initial = "SELECT * FROM inventory WHERE name LIKE ?"
        ...
        for i in products_list:
            ...
            #updating the stock
            sql = "UPDATE inventory SET stock=? WHERE name LIKE ?"
